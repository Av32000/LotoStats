...
async function getDataWithDate(date) {
  require('https').get('https://www.fdj.fr/jeux-de-tirage/loto/resultats/' + date, function (res) {
    ...
    res.on('end', async function () {
      ...
      if (buff.toString('utf8').split(`<span class="game-ball">`)[1] == null) {
        console.log(`Résultat indisponible pour le tirage du ${date}.`)
        return;
      }
  ...
  exec() {
    ...
    switch (date.getDay()) {
      ...
      case 6:
        ...
        break;
      default:
        console.log("Erreur inatendu concernant le jour de la semaine")
        return;
    }
    switch (date.getMonth()) {
      ...
      case 11:
        ...
        break;
      default:
        console.log("Erreur inatendu concernant le mois de l'année")
        return;
    }
    dotenv.config()
    getDataWithDate(`${day}-${date.getDate()}-${mois}-${date.getFullYear()}`)
  }
}
module.exports = GetTirageED